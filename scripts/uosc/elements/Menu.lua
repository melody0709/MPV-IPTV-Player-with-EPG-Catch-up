...
function Menu:init(data, callback, opts)
	...
	self.anchor_x = self.opts.anchor_x or 'center' -- Menu horizontal anchor: 'left', 'center', 'right'
	self.anchor_offset = self.opts.anchor_offset or 0 -- Additional offset for anchor position
...
-- 计算从根菜单到指定菜单的总宽度（包括间距）
function Menu:calculate_width_to_menu(menu)
	local width = menu.width + self.padding * 2
	local m = menu
	while m.parent_menu do
		width = width + m.parent_menu.width + self.padding * 2 + self.gap
		m = m.parent_menu
	end
	return width - (menu.width + self.padding * 2) -- 减去自身宽度，只保留左侧累积宽度
end

-- Updates element coordinates to match padding box of currently open (sub)menu.
function Menu:update_coordinates()
	local ax
	local anchor_x = self.anchor_x or 'center'
	local anchor_offset = self.anchor_offset or 0
	
	if anchor_x == 'left' then
		-- 左对齐模式：根菜单靠左，子菜单依次向右展开
		if self.current.is_root then
			ax = round(self.padding + anchor_offset) + self.offset_x
		else
			-- 子菜单：基于根菜单的左对齐位置 + 累积宽度
			local root_ax = round(self.padding + anchor_offset)
			local accumulated_width = self:calculate_width_to_menu(self.current)
			ax = root_ax + accumulated_width + self.offset_x
		end
	elseif anchor_x == 'right' then
		ax = round(display.width - self.current.width - self.padding * 2 - anchor_offset) + self.offset_x
	else -- center (default)
