...
**`scripts/epg/`** 是主要业务逻辑（模块化文件夹脚本），包含：

- `main.lua`：全局状态初始化、所有 mp.* 事件/消息/按键绑定
- `utils.lua`：字符串工具、UTF8、OSD、时间日期换算、拼音搜索
...
- 想在同一频道组内快速上下切台：直播时按 `PgUp` / `PgDn`；如果正在看回看，按键只会提示“回看中不支持组内切台”
