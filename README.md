...
- ✅ **悬停与点击交互修复** - 频道悬停可级联展开到当日 EPG 并定位当前时段；日期桶仅点击切换，修复点击两次才展开
- ✅ **菜单宽度稳定性修复** - 新增菜单级最大宽度约束，支持锁定三级日期桶宽度
- ✅ **频道搜索稳定性修复** - 修复搜索崩溃与多音字映射不稳定，`gd` 可匹配 `广东`，`dg` 可匹配 `东莞`

### v1.6.1 (2026-03-23)

- ✅ **悬停展开定位优化** - 鼠标滑过频道自动展开三级 EPG 时，当前时段不仅会自动选中，还会自动滚动到菜单可视区域中间

### v1.6 (2026-03-23)

- ✅ **三级菜单宽度独立控制** - 支持在 `epg.conf` 分别设置一级/二级/三级菜单最小宽度
- ✅ **副标题字体可配置** - 支持在 `epg.conf` 设置二级/三级副标题字体大小
- ✅ **三级 EPG 菜单副标题化** - 节目标题放主标题，日期时间迁移到副标题

### v1.5 (2026-03-23)

- ✅ **新增频道当前节目副标题** - 二级频道菜单可在频道名下方显示当前正在播放的节目名
- ✅ **菜单样式瘦身** - 去掉频道副标题中的时间段，缩短二级菜单宽度
- ✅ **菜单视觉极简化** - 移除频道右侧 `回看/EPG` 提示、分组 `folder` 图标、默认子菜单 `>` 箭头
- ✅ **文档整理** - 同步 README、开发手册、uosc 修改记录与独立 `CHANGELOG.md`
